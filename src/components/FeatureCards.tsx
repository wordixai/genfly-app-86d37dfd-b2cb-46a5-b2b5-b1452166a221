...
export const FeatureCards = () => {
  ...
  return <div className="grid md:grid-cols-2 gap-6 lg:grid-cols-4">
      {features.map((feature, index) => <a key={index} className="h-full" href={feature.href}>
          <div className="group/card text-card-foreground rounded-xl transition-all duration-200 bg-card hover:border-slate-7 hover:text-slate-12 border shadow-sm hover:shadow-lg flex flex-col h-full justify-between overflow-hidden">
            <div className="bg-background shadow-none transition-transform group-hover/card:scale-105 group-hover/card:-translate-y-1 relative h-48 overflow-hidden rounded-t-xl border-b p-2 flex-shrink-0">
              <div className="absolute inset-0">
                ...
                {feature.visual === "providers" && <div className="absolute inset-[1rem] grid grid-cols-5 gap-x-3 gap-y-1 scale-105 z-10">
                    {Array.from({
                length: 25
              }).map((_, i) => <div key={i} className="size-9 transform hover:scale-110 hover:brightness-110 transition-all duration-500 ease-out opacity-85" style={{
                ...
                {feature.visual === "routing" && <div className="h-full flex items-center justify-center">
                    <div className="relative w-full max-w-52 flex flex-col items-center gap-y-2">
                      ...
                      <div className="flex justify-between w-full">
                        {["Google", "Anthropic", "Amazon"].map(provider => <div key={provider} className="flex items-center justify-center size-6 flex-shrink-0 rounded-full border border-border/50 shadow bg-muted p-0.5">
                            <div className="overflow-hidden rounded-full">
                ...
                {feature.visual === "performance" && <div className="relative flex h-full items-center justify-center">
                ...
                {feature.visual === "security" && <div className="h-full flex items-center justify-center">
            ...
            <div className="flex flex-col gap-2 px-6 py-4 h-full">
              <div className="flex flex-col gap-2 h-full">
                <h3 className="text-2xl font-semibold group-hover/card:text-slate-12 transition-colors duration-200">One API for Any Model{feature.title}
                </h3>
